...
import java.awt.Graphics2D;import java.awt.event.ActionEvent;
...
public class Plansza extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == multiplayer) {
			...
			int decyzja = JOptionPane.showConfirmDialog(null, "Chcesz rozpoczac rozgrywke sieciowÂą?", "Confirm Dialog",
					JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
			if (decyzja == JOptionPane.YES_OPTION) {
				adresIP = JOptionPane.showInputDialog(poleIP, "Wpisz adres IP");
				try {
					if (adresIP.isEmpty()) {
						JOptionPane.showMessageDialog(null, "BÂłĂŞdny adres lub pusty!");
					} else {
